...
class ScreenCaptureService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        if (intent == null) {
            // Handle the case where the service is restarted by the system without a valid intent
            stopSelf() // Or decide on a different recovery strategy
        ...
        if (ScreenCaptureService.Companion.isStartCommand(intent)) {
            ...
            val data: Intent? = intent.getParcelableExtra<Intent>(ScreenCaptureService.Companion.DATA)
            if (resultCode == Activity.RESULT_OK && data != null) {
                startProjection(resultCode, data)
            } else {
                // Failed to get necessary data for projection
                sendAgentConsole("Screen capture failed: Missing permissions or data.")
                stopForeground(true) // Remove notification
                stopSelf()
            }
        } else if (ScreenCaptureService.Companion.isStopCommand(intent)) {
            stopProjection()
            stopForeground(true) // Remove notification if it was started
            stopSelf()
        } else {
            stopForeground(true) // Remove notification if it was started
            stopSelf()
