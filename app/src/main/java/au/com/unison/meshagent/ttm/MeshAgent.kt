...
class MeshUserInfo(useridParam: String, val realname: String?, val image: Bitmap?) {
    //val userid: String = userid
    //val realname: String? = realname
    //val image: Bitmap? = image
    init {
        println("MeshUserInfo: $useridParam, $realname")
    }
}
...
class MeshAgent(parent: MainActivity, host: String, certHash: String, devGroupId: String) : WebSocketListener() {
    ...
    private fun getUnsafeOkHttpClient(): OkHttpClient {
        ...
        val trustAllCerts = arrayOf<TrustManager>(object : X509TrustManager {
            ...
            override fun checkServerTrusted(chain: Array<out X509Certificate>?, authType: String?) {
                val certificate = chain?.firstOrNull() // Get the first certificate, or null if chain is null or empty
                val encodedCertificate: ByteArray? = certificate?.encoded

                if (encodedCertificate != null) {
                    serverTlsCertHash = MessageDigest.getInstance("SHA-384").digest(encodedCertificate)
                } else {
                    // Handle the case where the certificate or its encoding is not available
                    println("Error: Could not get encoded certificate from the chain in checkServerTrusted.")
                    serverTlsCertHash = null // Explicitly set to null or handle as an error state
    ...
    override fun onOpen(webSocket: WebSocket, response: Response) {
        ...
        val currentServerTlsCertHash = serverTlsCertHash
        ...
        if (currentServerTlsCertHash != null && currentNonce != null) {
            ...
            println("Error: serverTlsCertHash or nonce is null in onOpen. Cannot authenticate.")
            stopSocket() // Or some other error handling
            UpdateState(0) // Indicate disconnection or error
        }
    }
    ...
    override fun onMessage(webSocket: WebSocket, bytes: ByteString) {
        try {
            ...
            val cmd : Int = (bytes[0].toInt() shl 8) + bytes[1].toInt() // Use 'bytes'
            ...
            when (cmd) {
                1 -> {
                    ...
                    serverNonce = bytes.substring(50).toByteArray() // Use 'bytes'
                    ...
                    header[1] = 2
                    val certLen = agentCertificate!!.encoded.size
                    val agentCertLenBytes = ByteArray(2)
                ...
                2 -> {
                    ...
                    val xcertLen: Int = (bytes[2].toUByte().toInt() shl 8) + bytes[3].toUByte().toInt()
                    val xcertBytes = bytes.substring(4, 4 + xcertLen)
                    val xagentCertificate = CertificateFactory.getInstance("X509").generateCertificate(
                    ...
                    val pkey = xagentCertificate.publicKey as RSAPublicKey
                    val serverid = MessageDigest.getInstance("SHA-384").digest(pkey.encoded.toByteString().substring(24).toByteArray())
                    ...
                    val signBlock: ByteArray? = serverTlsCertHash!!.plus(nonce!!).plus(serverNonce!!)
                    val sig = Signature.getInstance("SHA384withRSA")
                    ...
                    val devGroupIdBytes: ByteArray = Base64.decode(devGroupId.replace('$', '/').replace('@', '+'), Base64.DEFAULT)
