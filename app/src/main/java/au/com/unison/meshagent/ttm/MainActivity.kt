...
class MainActivity : AppCompatActivity() {
        ...
        registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
            if (result.resultCode == Activity.RESULT_OK) {
                ...
                if (data != null) {
                    ...
                    // You would typically pass this data to your ScreenCaptureService here
                    // For example:
                    val serviceIntent = ScreenCaptureService.getStartIntent(this, result.resultCode, data)
                    startService(serviceIntent)

                    // Update your g_ScreenCaptureService or relevant state if needed
                    // For instance, if ScreenCaptureService sets a global variable on start:
                    // g_ScreenCaptureService = ... (this logic might be inside your service)

                }
            } else {
                // Screen capture permission was denied or request was cancelled
                showToastMessage("Screen capture permission denied.")
            }
        }
    ...
    fun openUrl(xpageUrl: String) : Boolean {
        ...
        if (visibleScreen == 1) {
            ...
            supportFragmentManager.setFragmentResult(
                WEBVIEW_FRAGMENT_REQUEST_KEY,
                bundleOf(
                    "action" to ACTION_NAVIGATE, // Custom action identifier
                    BUNDLE_URL_KEY to xpageUrl
                )
            )
        }
        return true
    ...
    fun returnToMainScreen() {
        this.runOnUiThread {
            if (visibleScreen == 2) {
                ...
                supportFragmentManager.setFragmentResult(
                    WEBVIEW_FRAGMENT_REQUEST_KEY,
                    bundleOf("action" to ACTION_EXIT) // Custom action identifier
                )
    ...
    companion object {
        ...
        const val WEBVIEW_FRAGMENT_REQUEST_KEY = "webViewFragmentRequestKey" // For Fragment Result API
        const val ACTION_NAVIGATE = "actionNavigate"
        const val ACTION_EXIT = "actionExit"
        const val BUNDLE_URL_KEY = "url"
    }
}